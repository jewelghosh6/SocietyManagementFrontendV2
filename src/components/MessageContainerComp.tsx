...
const MessageContainerComp = () => {
    ...
    useLayoutEffect(() => {
        console.log("chatEventKey", chatEventKey);
        ...
        if (!selectedChat.participants) {
            axiosInstance.get(`/chat/get-conversation-info?chatEventKey=${chatEventKey}`)
                .then(res => {
                    ...
                    fetchMessages(res.data.data.participants.sender.conversation_id)
                }).catch(err => console.log(err));
        }
        console.log("conversationDettail.participants", typeof conversationDettail.participants);

        if (selectedChat.participants) fetchMessages(selectedChat.participants.sender['conversation_id'])

        //Make API call and retrive old messages for this conversation

        socket.on('direct_message', (newMessage) => {
            // console.log({ newMessage }, chatEventKey === newMessage.chatEventKey);
        ...
        return () => {
            socket.off('direct_message', () => console.log("socket disconnected"));
        };

    }, [])

    const fetchMessages = async (conversation_id: any) => {
        try {
    ...
    return (
        <>
            <div className="bg-black-alpha-100 conversation_detail_header py-2 d-flex z-1 align-items-center ">
                ...
                <div>
                    <span>{conversationDettail.participants?.recipient[0]['user']["first_name"] + " " + conversationDettail.participants?.recipient[0]['user']["last_name"]}</span>
                </div>
            </div>
